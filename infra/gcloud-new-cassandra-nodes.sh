...
do
  ...
  declare -r cassandra_internal_ip=$(gcloud --project="akka-gcp" compute instances list | grep "cassandra-01" | head -n1 | awk '{ print $4 }')
  declare -r cassandra_external_ip=$(gcloud --project="akka-gcp" compute instances list | grep "cassandra-01" | head -n1 | awk '{ print $5 }')
  ...
  cat nodes/cassandra.json.template |
    ...
    sed "s/CASSANDRA_SEED_IP/$cassandra_internal_ip/g" |  
