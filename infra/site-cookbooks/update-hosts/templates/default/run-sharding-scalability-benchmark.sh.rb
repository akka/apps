...
run() {
  ...
  ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no $IP2 "
    ...
    cd /home/akka/apps
    /home/akka/sbt -Daeron.term.buffer.length=4194304 -J-XX:+PrintGCDetails -J-XX:+PrintGCTimeStamps -J-Xms4G -J-Xmx4G -Dsbt.log.noformat=true '; project sharding; runMain $CLAZZ' > $LOGS2/log-sharding.txt
  " &
...
echo "running test...!"
for ip in $(cat /home/akka/multi-node-test.hosts); do
  run $ip "com.lightbend.akka.bench.sharding.scalability.ShardingActorCountScalabilityApp" 
done
